...
def write_out_cluster_as_fa(member, all_sequences,
    ...
    try:
        ...
        ValueError  # we will let this pass for now
    return True
    ...
    for line in cluster_file:
        ...
        if number_of_reads_hitting_species >= 1 and db_species > 0:
            ...
            if len(cluster_line) > min_novel_cluster_threshold:
                ...
                if db_species < 1:
                    print ("im here")
                    out_novel = "%s/novel_d%s/novel%d_len%d.fasta" %(working_directory,
                                                                     v,
                                                                     cluster_number,
                                                                     len(cluster_line))
